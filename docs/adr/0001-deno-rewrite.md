...
- **Formatter semantics differ from `mdformat`** (prose wrapping, table alignment, frontmatter). Mitigated by pinning the `deno fmt` flags that make the port equivalent (`--no-config`, `--prose-wrap never`), accepting a one-time docs reformat, and porting `test_fmt` to `deno fmt` goldens. The shielding layer this entry originally promised was probed away in phase 7: frontmatter, `<!-- sparql:start -->` blocks, wikilinks, `%wiki.*%` tokens, and GFM alignment all survive `deno fmt` verbatim, so the port does not shield. What it cannot configure is emphasis (`*x*` → `_x_`), which is why `fmt-check-docs` is a `known` case by construction rather than a disagreement to fix.
