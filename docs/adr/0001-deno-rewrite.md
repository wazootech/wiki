...
| `mdformat` | **`dprint-plugin-markdown`** via `@dprint/formatter` | Reverses #273's recommendation; one-time docs reformat accepted. Every plugin is pinned to the exact version Deno 2.9.6 bundles, and the port's output is `deno fmt`'s output byte for byte on all 87 docs pages ([evidence](../../probes/fmt-dprint/FINDINGS.md)) |
| `jinja2` | `nunjucks` | |
...
- **Formatter semantics differ from `mdformat`** (prose wrapping, table alignment, frontmatter). Mitigated by pinning the plugin versions that make the engine equivalent to the `deno fmt` it replaced — so the cutover is output-neutral rather than a second reformat — accepting a one-time docs reformat, and porting `test_fmt` to measured goldens. The shielding layer this entry originally promised was probed away in phase 7: frontmatter, `<!-- sparql:start -->` blocks, wikilinks, `%wiki.*%` tokens, and GFM alignment all survive verbatim, so the port does not shield. What it cannot configure is emphasis (`*x*` → `_x_`), which is why `fmt-check-docs` is a `known` case by construction rather than a disagreement to fix.
