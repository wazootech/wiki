...
class TestWikiPaths(unittest.TestCase):
    ...
    def test_filename_pattern_is_full_match(self) -> None:
        with TemporaryDirectory() as tmpdir:
            ...
            config = WikiConfig(vault={"filename_pattern": r"[A-Za-z0-9_()-]+\.md"})
    ...
    def test_excluded_markdown_files_do_not_create_routes(self) -> None:
        with TemporaryDirectory() as tmpdir:
            ...
            config = WikiConfig(vault={"inputs": [wiki], "exclude": ["wiki/drafts/**"]}, config_root=root)
    ...
    def test_asset_manifest_preserves_config_root_relative_path(self) -> None:
        with TemporaryDirectory() as tmpdir:
            ...
            config = WikiConfig(vault={"assets": [root / "assets"]}, config_root=root)
    ...
    def test_asset_excludes_control_copied_files(self) -> None:
        with TemporaryDirectory() as tmpdir:
            ...
            config = WikiConfig(vault={"assets": [assets], "exclude": ["assets/secret.txt"]}, config_root=root)
    ...
    def test_asset_links_are_checked_under_internal_links(self) -> None:
        with TemporaryDirectory() as tmpdir:
            ...
            config = WikiConfig(vault={"inputs": [wiki], "assets": [root / "assets"]}, config_root=root)
    ...
    def test_missing_asset_link_is_reported(self) -> None:
        with TemporaryDirectory() as tmpdir:
            ...
            config = WikiConfig(vault={"inputs": [wiki], "assets": [assets]}, config_root=root)
